...
func newNamedTempStore(t *testing.T, titles ...string) (*store.Store, string) {
	...
	for _, title := range titles {
		if _, err := s.Add(title, "", nil, nil, task.PriorityNone); err != nil {
...
func TestMultiStoreFromConfig_NoCollections(t *testing.T) {
	...
	if _, addErr := s.Add("Hello", "", nil, nil, task.PriorityNone); addErr != nil {
...
func TestMultiStoreFromConfig_NamedPrimary(t *testing.T) {
	...
	if _, addErr := primaryS.Add("Primary task", "", nil, nil, task.PriorityNone); addErr != nil {
		t.Fatalf("Add primary: %v", addErr)
	}
	collS := store.New(collRoot)
	if _, addErr := collS.Add("Coll task", "", nil, nil, task.PriorityNone); addErr != nil {
